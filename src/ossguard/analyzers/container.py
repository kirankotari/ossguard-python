...
def scan_containers(project_path: str | Path) -> ContainerReport:
    ...
    for df_path in dockerfiles:
            ...
            for i, line in enumerate(lines, 1):
                if compiled.search(line):
                    findings.append(ContainerFinding(
                        file=rel_path, line_number=i, rule_id=rule_id,
        ...
        if not re.search(r'^\s*HEALTHCHECK\b', content, re.MULTILINE):
            findings.append(ContainerFinding(
                file=rel_path, line_number=0, rule_id="DL-050",
                severity="low", description="No HEALTHCHECK instruction found",
        ...
        if not re.search(r'^\s*USER\b', content, re.MULTILINE):
            findings.append(ContainerFinding(
                file=rel_path, line_number=0, rule_id="DL-010",
                severity="high",
        ...
        if from_count == 1:
            findings.append(ContainerFinding(
                file=rel_path, line_number=0, rule_id="DL-070",
                severity="low",
    ...
    if files_scanned > 0 and not (path / ".dockerignore").exists():
        findings.append(ContainerFinding(
            file=".dockerignore", line_number=0, rule_id="DL-060",
            severity="medium",
